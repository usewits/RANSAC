...
class RANSAC {
	...
	public static void main(final String[] args) throws FileNotFoundException, IOException {
		...
		// first make sure the parameters are correct. This is just conversion logic, read on!
		...
		// Here is where the main functionality start!
		final List<Point> points = RANSAC.readCSV("../data/"+filename+".csv");
		
		//Scale all points to a 1x1 box 
		//Find bounding box
		...
		//Will find at most 100 models
		boolean appendToFile=false; // this creates a new file, consequent uses use =true.
		for(int i=0; i<100; i++) {
			final List<Model> models = RANSAC.doRansac(points,n_iters,epsilon);
			final Model best = models.get(0);
			final Model updatedBest = Model.improveAnulusApprox(best);
